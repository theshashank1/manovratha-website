    ...
    return (
        <section ref={containerRef} className="relative pt-24 pb-8 md:pt-48 md:pb-32 overflow-hidden min-h-[80vh] md:min-h-[90vh] flex items-center px-4 md:px-6">
            ...
            <div className="max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
                ...
                {/* Mobile: Hidden pillars, show only image. Desktop: Full view with floating cards */}
                <div className="relative h-[300px] sm:h-[400px] md:h-[600px] w-full perspective-[2000px] order-1 lg:order-2 mb-4 lg:mb-0 overflow-visible">
                    {/* Main Image Container */}
                    <MotionDiv
                        initial={{ opacity: 0, rotateY: 15, x: 50 }}
                        animate={{ opacity: 1, rotateY: -5, x: 0 }}
                        transition={{ duration: 1.2, ease: "easeOut" }}
                        className="absolute inset-0 flex items-center justify-center z-10 scale-[0.7] sm:scale-[0.8] md:scale-100"
                    >
                        <div className="relative w-[280px] sm:w-[320px] md:w-[400px] h-[350px] sm:h-[420px] md:h-[550px] group">
                            ...
                            {/* 4 Pillars Floating Cards - Hidden on mobile, visible on tablet+ */}
                            <div className="hidden sm:block">
                                {[
                                    { title: "Incubation", icon: "self_improvement", color: "bg-indigo-100 text-indigo-600", pos: "-top-4 -right-8 md:-top-8 md:-right-12", delay: 0.5 },
                                    { title: "Sanctuary", icon: "spa", color: "bg-emerald-100 text-emerald-600", pos: "top-1/3 -left-8 md:-left-24", delay: 0.7 },
                                    { title: "Community", icon: "handshake", color: "bg-amber-100 text-amber-600", pos: "bottom-1/4 -right-8 md:-right-20", delay: 0.9 },
                                    { title: "Tech Support", icon: "laptop_chromebook", color: "bg-blue-100 text-blue-600", pos: "-bottom-4 -left-6 md:-bottom-6 md:-left-12", delay: 1.1 },
                                ].map((pillar, i) => (
                                    <MotionDiv
                                        key={i}
                                    ...
                                    >
                                        ...
                                        <div>
                                            ...
                                            <p className="font-bold text-brand-dark text-[11px] sm:text-xs md:text-base leading-tight">{pillar.title}</p>
                                        </div>
                                    </MotionDiv>
                                ))}
                            </div>
