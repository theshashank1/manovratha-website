        ...
        <div className="flex flex-col w-full max-w-2xl mx-auto py-4 md:py-6">
            ...
            {!result ? (
                ...
                <motion.div 
                    ...
                    className={`bg-white p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] border text-center shadow-card ${
                        resultType === 'optimal' ? 'border-emerald-100 bg-emerald-50/30' : 
                        resultType === 'functional' ? 'border-amber-100 bg-amber-50/30' : 
                        'border-rose-100 bg-rose-50/30'
                    }`}
                >
                    <div className={`w-20 h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center mx-auto mb-6 md:mb-8 shadow-sm ${
                        resultType === 'optimal' ? 'bg-emerald-100 text-emerald-700' : 
    ...
    return (
        <div className="h-full flex flex-col max-w-5xl mx-auto py-4">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 md:mb-8 border-b border-gray-100 pb-6 shrink-0 gap-4">
                ...
                <button 
                    onClick={handleCopy}
                    className={`px-5 py-3 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-md w-full sm:w-auto justify-center ${
                        copied ? 'bg-emerald-600 text-white' : 'bg-brand-primary text-white hover:bg-brand-dark'
                    }`}
                >
                        ...
                        <div className="flex flex-wrap gap-2 md:gap-3">
                            {cat.options.map((opt) => (
                                <button
                                    ...
                                    className={`px-4 py-2 md:px-5 md:py-2.5 rounded-xl text-xs md:text-sm font-medium transition-all border ${
                                        mseData[cat.id] === opt 
...
const WellnessHub: React.FC<WellnessHubProps> = ({ onNavigate }) => {
            ...
            <Reveal>
                <div className="bg-white rounded-[2rem] md:rounded-[3rem] p-8 md:p-14 border-l-8 border-brand-primary shadow-soft flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12 text-center lg:text-left">
                    ...
                    <div className="flex flex-col sm:flex-row flex-wrap gap-4 justify-center w-full lg:w-auto">
                        <button onClick={() => onNavigate('contact')} className="text-brand-primary font-bold hover:underline px-6 py-3 text-lg">Suggest a Tool</button>
                        <button onClick={() => onNavigate('contact')} className="bg-brand-dark text-white px-8 md:px-10 py-3 md:py-4 rounded-2xl font-bold text-base shadow-md hover:bg-brand-primary transition-colors">Contribute Research</button>
                    </div>
                </div>
            </Reveal>
